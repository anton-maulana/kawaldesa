    ...
    class UserCtrl {
        ...
        loadUser(id: string) {
            ...
            Services.UserController.Get(id).then((user) => {
                ...
                } console.log("region Pairs",ctrl.regionPairs);
            });
        ...
        removeScopes() {
            angular.forEach(this.checkedIndex, x => {
                this.regionPairs.splice(this.regionPairs.indexOf(x), 1)
            })
            this.checkedIndex=[];
        }

        indexChecked(regionPair) {
            if (this.checkedIndex.indexOf(regionPair) === -1)
                this.checkedIndex.push(regionPair);
            else
                this.checkedIndex.splice(this.checkedIndex.indexOf(regionPair),1);
        }
    }
